...
class MyEafeaAlgorithm(Algorithm):
    """An example for a EAFEA(B) hybridization algorithm."""
    ...
    def solve(self, process: Process) -> None:
        ...
        This is an optimization process of EAFEA(B) hybridization algorithm based on EA algorithm with reversing operator.
        ...
        while not should_terminate():
            ...
            if useFFA:
                ...
                if yd <= yc:  # if FEA finds a better solution
                    yc = yd
                    np.copyto(xc, xd)
                register(xd, yd)
            else:
