...
with open("eil101_cost.txt","w")as f:
        f.write(str(cost_tsp))
        index_max = np.argmax(cost_tsp, axis=1)# 其中，axis=1表示按行计算
        #print(index_max.shape)
        max = cost_tsp[range(cost_tsp.shape[0]), index_max]
        #print(max)
        f.write('\n'+str(max))
        upperbound=sum(max)
        f.write(str(upperbound))
print(upperbound)
...
def rev(dx):       # i j from cost_tsp 交换x中的ij得到新的x2 并计算x2的回路长度 返回新dx
        ...
        #dx['tour_total_distance'] = dx['tour_total_distance'] - cost_tsp[x[((i-1)+101)%101]-1][x[i]-1] - cost_tsp[x[j]-1][x[(j+1)%101]-1] + cost_tsp[x[i]-1][x[(j+1)%101]-1] + cost_tsp[x[((i-1)+101)%101]-1][x[j]-1]
#-----------------------            
        xa =x[:i]
        xb =x[j+1:]
        xij = x[i:j+1]
        ...
        x2 = xa + xji +xb
        dx['x'] = x2
        #dx['tour_total_distance'] = get_tour_cost(x2,cost_tsp)
        return dx
...
while (count < 10000):
   ...
   yc = int(yc)
   yn = int(yn)
   if(dxn['tour_total_distance'] <= dxb['yb']):
    dxb['xb']=dxn['xn']
    dxb['yb']=dxn['tour_total_distance']
    H[yc] = H[yc]+1
    H[yn] = H[yn]+1
    if(H[yn]<=H[yc]):
        dxc['xc']=dxn['xn']
